...
public class TUpmed extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(int i = 0; i < list.size(); i++) {
			...
			if(flag >= 0) {
				response.sendRedirect("/hospital/hos_admin/Managemed");
			}else {
				out.println("<script>");
				out.println("alert('ÐÞ¸ÄÊ§°Ü£¡');window.location.href='hos_admin/upmed.jsp' onClick=¡±javascript :history.back(-1);¡±");
				out.println("</script>");
			}
		}
