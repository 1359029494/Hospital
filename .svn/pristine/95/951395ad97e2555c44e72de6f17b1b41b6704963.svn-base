...
<body>
					...
					<div class="ydc-column-user">
						...
						<div class="ydc-user-info">
							...
							<div class="ydc-user-info-func ydc-flex">
								<span class="ydc-tag">管理员</span> <a href="admlogin.html">退出</a>
							</div>
						</div>
					</div>
				</div>
								...
								<div class="ydc-pane" style="display: block;">
									<div class="ydc-release-form-group ">
										<div class="ydc-group-input clearfix"
											...
											<input type="text" id="txtname" placeholder="请输入关键词进行搜索"
												style="width: 91.333%">
										...
										<c:forEach items="${list }" var="s">
											<div class="ydc-asset-img-list">
												<div class="ydc-asset-img-list-img">
													<img
														src="<%=request.getContextPath() %>/hos_admin/admin/${s.drphoto }">
												</div>
												<div class="ydc-asset-img-list-til">${s.drname }</div>
												<div class="ydc-asset-img-list-del">
													<a href="Upmed?drid=${s.drid }">
														<button>编辑</button>
													</a>
													<button del="${s.drid }" class="delete">删除</button>
												</div>
											</div>
										</c:forEach>
									</div>
									<div class="ydc-pagination">
										<ol>
											...
											<c:if test="${pagenow!=1 }">
												<li class="ydc-previous-item">
													<a href="Managemed?pagenow=${pagenow-1 }"><button class="ydc-previous-item-btn-medium">
														<span>上一页</span>
													</button></a>
												</li>
											</c:if>
											...
											<li class="ydc-item-quick">第
												<div class="ydc-item-quick-kun">
													<form action="Managemed" method="post">
	...
	<script type="text/javascript">
        $('.ydc-group-button').click(function(){
				var drname = document.getElementById("txtname").value;
				$.post("/hospital/Findmed",{drname:drname},function(data){
					$('.ydc-asset-img').html('');
						var tpml='';
						tpml+='<div class="ydc-asset-img-list">'
		                       +'<div class="ydc-asset-img-list-img">'
		                       +'<img src="admin/'+data.drphoto+'">'
		                       +'</div>'
		                       +' <div class="ydc-asset-img-list-til">'+data.drname+'</div>'
		                       +'<div class="ydc-asset-img-list-del">'
		                       +' <a href="Upmed?drid='+data.drid+'"><button>编辑</button></a>'
		                       +'  <button>删除</button>'
		                       +'</div>'
		                       +'</div>';	
	    ...
	    for(var i=0;i<del.length;i++){
	    	var ali=del[i];
	    		...
	    		del[i].onclick=function(){
	    	    	...
	    	    	if(flag == true){
	    	    		window.location.href="/hospital/hos_admin/Delectmed?drid="+id;}
	    		}
