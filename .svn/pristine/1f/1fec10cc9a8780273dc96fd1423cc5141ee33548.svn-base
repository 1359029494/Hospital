...
public class OrdersDAO {
	...
	//通过订单编号 直接查询该订单的所有信息
	public List<Orders> findBynumber(String ornumber) throws Exception{
	...
	//根据订单编号查询药品信息
		...
		public int mergetojiaofei(Orders o) throws Exception{
			String sql=" update Orders set orstate=? where ornumber=? ";
			Object []params={
					o.getOrstate(),
					o.getOrnumber()
			};
		...
		//根据订单编号，查询订单状态
		public int selectOrstate(String ornumber) throws Exception{
			String sql="select *  from orders where ornumber=?";
			Object []params={
					ornumber
			};
			...
			DBManager db=new DBManager();
			List<Orders> list=db.executeQueryObjectList(sql, params, mapper);
			return list.get(0).getOrstate();
		}	
		//根据订单编号删除订单信息
		public int deleteornumber(Orders o) throws Exception{
			String sql = "delete from Orders where ornumber = ?";
			Object []params={
					o.getOrnumber()
			};
			DBManager db=new DBManager();
